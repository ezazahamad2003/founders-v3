...
async def stream_chat_with_files(
    ...
    *,
    ...
    OPENAI_FILES_API_MIMES = {
        "application/pdf",
        "application/msword",
        "application/vnd.openxmlformats-officedocument.wordprocessingml.document",
    }
    OPENAI_FILES_API_EXTENSIONS = {".pdf", ".doc", ".docx"}
    all_supported_by_files_api = all(
        (f.mime_type and f.mime_type in OPENAI_FILES_API_MIMES)
        or (f.original_name and any(f.original_name.lower().endswith(ext) for ext in OPENAI_FILES_API_EXTENSIONS))
        for f in files_meta
    )

    logger.info(f"[FILE_CONTEXT] all_have_ids={all_have_ids}, all_supported_by_files_api={all_supported_by_files_api}")

    # Preferred path: Files API for documents with openai_file_ids (PDF, DOC, DOCX)
    if files_meta and all_have_ids and all_supported_by_files_api:
        logger.info(f"[FILE_CONTEXT] >>> Using OpenAI Files API path (documents with file_ids) - STREAMING")
        try:
            ...
            logger.warning(
                "OpenAI Files API failed for documents, falling back to text extraction: %s",
                e,
            )
...
async def deep_research(
    ...
    *,
    ...
    system_prompt = get_system_prompt(prompt_mode="general")
